...
public:
    ...
    vector<string> busquedaPorProfundidad () {
        ...
        while (true) {
            if (!resultado->getEstaEstancado()) {
                ...
                sort(proximos_hijos.begin(),proximos_hijos.end(), [this](string a, string b) {
    ...
    bool compararfH(string i1, string i2)
    {
        return (this->fH(i1) < this->fH(i2));
    }

    int fH(string cadena){
        int contador = 0;
        for (int i = 0; i < cadena.size(); i++){
            if (this->resultado_esperado[i] != cadena[i])
                contador++;
        }
        return contador;
    }
